...
function createDiscription() {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            var responseObject = JSON.parse(xhr.responseText);
            var productName = responseObject.AppleProductCase[itemNumber].Productname;
            var discription = responseObject.AppleProductCase[itemNumber].description;
            var color = responseObject.AppleProductCase[itemNumber].color;
            var price = responseObject.AppleProductCase[itemNumber].Price;
            var id = responseObject.AppleProductCase[itemNumber].Id;
            var item = responseObject.AppleProductCase[itemNumber];
...
function createImages() {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            var responseObject = JSON.parse(xhr.responseText);
            var image = responseObject.iphonecase;
