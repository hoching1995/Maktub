...
function createDiscription() {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            var responseObject = JSON.parse(xhr.responseText);
            var productName = responseObject.Women[itemNumber].Productname;
            var discription = responseObject.Women[itemNumber].description;
            var color = responseObject.Women[itemNumber].color;
            var price = responseObject.Women[itemNumber].Price;
            var id = responseObject.Women[itemNumber].Id;
            var item = responseObject.Women[itemNumber];
            console.log(discription[0]);
            const div = document.createElement("div");
...
function createImages() {
    ...
    const createMainImage = document.querySelector('.col-2');
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            var responseObject = JSON.parse(xhr.responseText);
            var image = responseObject.BucketBag;
            // create the main image 
            const div = document.createElement("div");
            createMainImage.innerHTML =
                `
            <img src="${image[0].src}" alt="" width="800%" id="product-img">
            ...
            for (var i = 0; i < image.length; i++) {
                ...
                div2.innerHTML += `<img src="${image[i].src}" alt="" width="100%" class="small-img">`
